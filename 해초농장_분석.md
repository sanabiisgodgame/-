...
## 2026-05-06 배경 3단 레이어 적용 사전 분석 (v8.6 기준)
### 전문가용(구조/라인맵/데이터흐름)
- 메인 게임 루트는 `#farm`이며, 내부 렌더 순서는 배경(`#uwbg`, `#particle-canvas`) → HUD(`#farm-hd`) → 노드(`#nodes-grid`) → 로봇/이벤트 오버레이 순서다.
- 기존 `#farm`은 단일 radial 배경만 사용해 깊이감이 약했고, 시각 효과는 `#uwbg`(DOM 버블/광선) + `#particle-canvas`(캔버스 파티클)에 분산되어 있었다.
- 따라서 CSS 기반 3단 배경은 `#farm-bg-mid`, `#farm-bg-top`를 `#farm` 직계 레이어로 추가하고, 기존 상호작용 레이어와 충돌하지 않도록 `pointer-events:none`, `z-index` 분리를 우선 적용하는 것이 안전하다.
- 모바일(768px 이하)은 GPU/페인트(화면 다시 그리기) 부담을 줄이기 위해 파티클 레이어를 축소하고 보조 레이어(`::before`)를 끄는 전략이 적합하다.

### 초보용(쉽게 설명)
- 게임 화면 본체는 `#farm` 상자예요.
- 여기에 배경을 3겹으로 깔면 바다 느낌이 더 좋아져요.
  1) 맨 아래: 바다 색 그라데이션(위 밝고 아래 어두움)
  2) 중간: 빛 번짐
  3) 맨 위: 작은 먼지/버블이 천천히 올라감
- 중요한 버튼/노드 클릭이 안 막히게, 배경 레이어는 클릭을 받지 않게(`pointer-events:none`) 해야 해요.
- 모바일은 효과를 반으로 줄여서 버벅임을 예방해요.

### 개선 후보(즉시 미반영)
- `#uwbg`(DOM 기반 배경)와 `#particle-canvas`(캔버스 기반 배경) 역할을 통합해 저사양 모드 토글을 제공하면 성능 제어가 더 쉬워질 수 있음.
